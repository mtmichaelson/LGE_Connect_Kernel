...
const u16 lg_default_pid	= 0x620C;
#define lg_default_pid_string "620C"
const u16 lg_ndis_pid = 0x620C;
#define lg_ndis_pid_string "620C"
#else
const u16 lg_default_pid	= 0x6204;
#define lg_default_pid_string "6204"
...
#endif
#ifdef CONFIG_LGE_USB_GADGET_SUPPORT_FACTORY_USB
char serial_number[MAX_STR_LEN] = "\0";
#else
...
{
// [START] taeyoung.chung : for TWO_MODEM_USB_DRIVER 2011.06.22
...
static void android_set_function_mask(struct android_usb_product *up)
{
	...
	list_for_each_entry(func, &android_config_driver.functions, list) {
...
#ifdef CONFIG_USB_SUPPORT_LGE_ANDROID_AUTORUN
		{
			...
			{
#if 0//def CONFIG_LGE_USB_GADGET_SUPPORT_FACTORY_USB //20110812 block NMEA (ADB) driver update issue(releated to the factory)
...
int android_set_pid_without_reset(const char *val, acc_cable_type usb_cable_type)
{
    ...
    pr_info("%s: Requested product id is 0x%lx\n", __func__, tmp);
...
#ifdef CONFIG_USB_SUPPORT_LGE_ANDROID_AUTORUN
extern u8 autorun_onlone;
int android_set_usb_mode(const char *val, struct kernel_param *kp)
{
	int ret = 0;
    static u8 first_chk = 1;
    if((first_chk == 1 && autorun_onlone))
    {
        first_chk = 0;
		return -EINVAL;
    }
...
// [START] seunghun.kim : for LG_USB_DRIVER 2011.03.25
	...
	/* if none cable booting, read NV manual_test_mode and set force Factory USB */
#if 0//def CONFIG_LGE_USB_GADGET_SUPPORT_FACTORY_USB
	lg_manual_test_mode = msm_get_manual_test_mode();
#endif
	
#if 1
	if (usb_cable_info==LT_CABLE_56K || usb_cable_info==LT_CABLE_130K)
#else
